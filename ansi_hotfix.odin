...
when #config(FIX_WINDOWS_ANSI, true) {
    ...
    __fix_windows_ansi__ :: proc() {
        ...
        if stdout_handle == windows.INVALID_HANDLE_VALUE {
            fmt.println("__fix_windows__ansi__: Failed to get standard output handle");
            return;
        }
    
        mode: u32;
        if !windows.GetConsoleMode(stdout_handle, &mode) {
            fmt.println("__fix_windows__ansi__: Failed to get console mode");
            return;
        }
    
        mode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;
        if !windows.SetConsoleMode(stdout_handle, mode) {
            fmt.println("__fix_windows__ansi__: Failed to set console mode (ENABLE_VIRTUAL_TERMINAL_PROCESSING)");
            return;
        }
    }
}
